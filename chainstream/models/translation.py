...
def load_t5(model_name="google/flan-t5-base", prefer_cuda=True):
    ...
    try:
        model = T5ForConditionalGeneration.from_pretrained(model_path, device_map="auto" if prefer_cuda and cuda.is_available() else "cpu")
...
def translate(text: str, target_language="English", translator = load_t5()) -> str:
    ...
    input_ids = translator[1](input_text, return_tensors="pt").input_ids.to(translator[0].device)
